defmodule RehoboamGraphQl.Resolver.Field do
  alias Potionx.Context.Service
  alias Rehoboam.Schemas.FieldService
  use Absinthe.Relay.Schema.Notation, :modern

  def collection(args, %{context: %Service{} = ctx}) do
    q = FieldService.query(ctx)
    count = FieldService.count(ctx)
  ...
  def data do
    Dataloader.Ecto.new(Rehoboam.Repo, query: &FieldService.query/2)
  end

  def delete(_, %{context: %Service{} = ctx}) do
    FieldService.delete(ctx)
    |> case do
      {:ok, %{field: res}} -> {:ok, res}
