defmodule RehoboamGraphQl.Resolver.Schema do
  alias Potionx.Context.Service
  alias Rehoboam.Schemas.SchemaService
  use Absinthe.Relay.Schema.Notation, :modern

  def collection(args, %{context: %Service{} = ctx}) do
    q = SchemaService.query(ctx)
    count = SchemaService.count(ctx)
  ...
  def data do
    Dataloader.Ecto.new(Rehoboam.Repo, query: &SchemaService.query/2)
  end

  def delete(_, %{context: %Service{} = ctx}) do
    SchemaService.delete(ctx)
    |> case do
      {:ok, %Rehoboam.Schemas.Schema{} = res} -> {:ok, res}
      {:ok, %{schema: res}} -> {:ok, res}
